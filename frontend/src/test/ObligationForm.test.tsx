...
describe("ObligationForm Component",()=>{
	...
	it("renders all core form input elements correctly",()=>{
		renderComponent();
	...
	it("displays validation error messages when submitting empty required fields",async ()=>{
		renderComponent();
		const submitButton = screen.getByRole("button",{ name: /log obligation/i });
		//submit when empty
		await userEvent.click(submitButton);
		// check shecma validation err
		await waitFor(()=>{
	...
	it("successfully updates fields and handles a clean submission flow", async () => {
		...
		await userEvent.type(screen.getByLabelText(/category/i), "Health");
