...
import { getReceiptScan, type ReceiptScan, type ReceiptExtraction, type ReceiptConfirmationResult, createReceiptObligation } from '../features/receipts/receiptsApi'
...
export default function ReceiptReviewPage() {
    ...
    const [confirmationType, setConfirmationType] = useState<'existing-occurrence' | 'new-obligation' | null>(null)

    const [isExpired, setIsExpired] = useState(() => {
        return initialScan ? new Date(initialScan.expiresAt).getTime() <= Date.now() : false
    })

    // cfor the scanning of a recipt that does not have an exisitng financial obligation 
    const [creatingObligation, setCreatingObligation] = useState(false)
    const [createObligationError, setCreateObligationError] = useState<string | null>(null)
    ...
    const handleConfirmed = useCallback((result: ReceiptConfirmationResult) => {
        setConfirmationType('existing-occurrence')
        setConfirmedResult(result)
    }, [])

    const handleCreateObligation = useCallback(async () => {
        if (!scan || creatingObligation) return

        setCreatingObligation(true)
        setCreateObligationError(null)

        try {
            const values = editedValues ?? getInitialReceiptValues(scan.extraction)

            const categories = await getCategories('OBLIGATION')
            const customCategory = categories.data.find(
                category => category.name === 'Custom'
            )

            if (!customCategory) {
                throw new Error('Custom category could not be found')
            }

            const result = await createReceiptObligation(
                scan.id,
                {
            ...
            setConfirmationType('new-obligation')
            setConfirmedResult(result)

        } catch (error) {
            console.error('Failed to create obligation from receipt:', error)
            setCreateObligationError('Unable to record this receipt. Please try again.')
        } finally {
            setCreatingObligation(false)
        }
    }, [scan, editedValues, creatingObligation])
    ...
    return (
        <main className="min-h-[100dvh] bg-[#F4FBF7] px-5 py-8 text-[#091828] dark:bg-[#0b1326] dark:text-white">
            <div className="mx-auto flex w-full max-w-xl flex-col gap-6">
                ...
                {confirmationType !== 'new-obligation' && (
                ...
                {confirmedResult && confirmationType === 'new-obligation' && (
                    <section className="rounded-3xl border-2 border-[#091828] bg-white p-6 shadow-[5px_5px_0_#091828] dark:border-[#060e20] dark:bg-[#131b2e] dark:shadow-[5px_5px_0_#060e20]">
                        <div className="flex flex-col items-center text-center">
                            <div className="flex size-14 items-center justify-center rounded-full bg-[#DCEFE8] text-2xl font-black text-[#10775F] dark:bg-[#0f4f42] dark:text-[#5eead4]">
                                ✓
                            </div>
                            ...
                            <p className="mt-2 text-sm leading-6 text-[#6b6375] dark:text-[#a0aec0]">
                                Your receipt has been recorded and the payment has been added.
                            </p>
                        </div>
                    </section>
                )}

                {createObligationError && (
                    <p
                        role="alert"
                        className="mt-3 rounded-2xl bg-[#FFD9E1] px-4 py-3 text-sm font-semibold text-[#AC2A5D] dark:bg-[#4B2635] dark:text-[#ffb1c5]"
                    >
                        {createObligationError}
                    </p>
                )}
