...
const CATEGORY_CONFIG: Record<string, {label: string; icon: React.ReactNode}>={
    CORE_MILESTONES: {label: "Core Milestones", icon: <Award01Icon size={20} />},
    SAVINGS_QUESTS: {label: "Savings Quests", icon: <PiggyBankIcon size={20} />},
    SPECIAL_EVENTS: {label: "Special Events", icon: <SparklesIcon size={20} />},
    PAYMENT: {label: "Payment", icon: <CreditCardIcon size={20} />},
    STREAK: {label: "Streak", icon: <Fire02Icon size={20} />},
    MILESTONE: {label: "Milestones", icon: <StarIcon size={20} />},
    KNOWLEDGE: {label: "Knowledge", icon: <BookOpen01Icon size={20} />},
}
...
function StickerIcon({iconKey, size = 36}: {iconKey: string; size?: number}){
    ...
    return <>{icons[iconKey] ?? <StarIcon size={size} />}</>
}
...
function CategorySection({category, badges, onSelect}:{
    ...
    const config = CATEGORY_CONFIG[category] ?? {label: category, icon: <StarIcon size={20}/>}
