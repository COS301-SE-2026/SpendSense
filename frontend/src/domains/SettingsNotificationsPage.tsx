...
export default function SettingsNotificationsPage(){
    ...
    return(
        <SubPageShell title="Notifications and Reminders" subtitle="What SpendSense will notify you about">
            ...
            {error && !loading && (
                <div>
                    <p className="text-sm text-[#AC2A5D]">Your reminder settings couldn't be loaded.</p>
                    <button
                        type="button"
                        ...
                        <div className="mt-3 flex gap-2">
                            {REMINDER_DAY_OPTIONS.map((d)=> (
                                <button
                                    ...
                                    disabled={saving}
                                    className={`rounded-full px-3 py-1.5 text-xs font-bold transition ${prefs.defaultReminderDaysBefore === d ? "bg-[#FFD8E6] text-[#AC2A5D]" : "bg-[#E3EAE6] text-[#6B6375] hover:text-[#091828]"}`}
                                >
                                    {d} {d === 1 ? "day" : "days"}
                                </button>
                            ))}
                        ...
                        {CHANNEL_ROWS.map((row)=> (
                            <div key={row.key} className="flex items-center gap-3 border-b border-[#E8E4F4] px-3 py-3 last:border-b-0">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-xs text-[#6B6375]">{row.description}</p>
                                </div>
                                <Toggle
