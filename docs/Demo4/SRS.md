...
| **QR-06** | The SpendSense API shall provide responsive performance during normal operation, with at least 95% of requests to a representative authenticated endpoint completing within 500 ms and fewer than 1% of requests failing. | Client-server architecture, RESTful backend API, layered NestJS architecture and asynchronous request handling   | **k6 response-time test**                                  | **p95 response time < 500 ms and request failure rate < 1%** / *actual p(95) response time = 436.13ms and failure rate = 0%*                                   | k6 run response-time.js [results](../../NFR-tests/k6-response-time.md)                                                                                                                                                       |
| **QR-07** | The SpendSense API shall maintain acceptable performance while servicing up to **100 concurrent** virtual users, with at least 95% of requests completing within 500 ms and fewer than 1% of requests failing.            | Client-server architecture, stateless REST API, modular backend services and containerised deployment            | **k6 staged load test**                                    | **100 concurrent virtual users, p95 response time < 500 ms and request failure rate < 1%** / *actual p95 response time = 372 ms and request failure rate = 0%* | k6 run load.js [results](../../NFR-tests/k6-load.md)                                                                                                                                                       |
