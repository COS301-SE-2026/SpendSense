...
export class ObligationsService {
  ...
  async create(userId: string, dto: CreateObligationDto) {
    ...
    if (
      dto.schedule.frequency === ScheduleFrequency.FIXED_INSTALLMENT &&
      !dto.schedule.totalOccurrences
    ) {
      throw new BadRequestException(
        'Total occurrences is required for fixed installments',
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (dto.schedule && activeSchedule) {
        await tx.paymentSchedule.update({
          ...
          data: {
            ...
            ...(dto.schedule.dayOfMonth !== undefined && {
      ...
      if (dto.regenerateFutureOccurrences && activeSchedule) {
        ...
        const newDates = generateOccurrenceDates(
          newSchedule.frequency ?? activeSchedule.frequency,
...
function generateOccurrenceDates(
  ...
  const dates: Date[] = [];
  const horizonEnd =
    endDate ?? addMonths(new Date(), OCCURRENCE_HORIZON_MONTHS);
...
function resolveFirstOccurrence(
  startDate: Date,
