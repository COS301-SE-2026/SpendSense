...
export class PaymentOccurrencesController {
  ...
  @ApiOperation({
    summary:
      'Get upcoming payment occurrences for dashboard and timeline views',
  })
  @ApiResponse({ status: 200, description: 'Upcoming occurrences returned' })
  @ApiResponse({ status: 401, description: 'Unauthorised' })
  async upcoming(
    @CurrentAuthUser() authUser: AuthUser,
    @Query() query: UpcomingOccurrencesDto,
  ) {
  ...
  @ApiOperation({
    summary:
      'Get one payment occurrence with obligation context, payment record, and score risk',
  })
