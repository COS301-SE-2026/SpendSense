...
describe('RemindersController', ()=>{
    let remindersService: jest.Mocked<Pick<RemindersService, 'getReminderPreferences'|'updateReminderPreferences'>>;
    ...
    beforeEach(()=>{
        ...
        controller = new RemindersController(remindersService as unknown as RemindersService,);
    });
    ...
    it('updates the authenticated users reminder preferences using the service', async()=>{
        ...
        await expect(controller.updateReminderPreferences(dto, authUser),).resolves.toEqual(updated);
    ...
    it('returns the authenticated users reminder preferences using the service', async()=>{
        ...
        await expect(controller.getReminderPreferences(authUser)).resolves.toEqual(preferences);

        expect(remindersService.getReminderPreferences).toHaveBeenCalledWith(authUser);
    });
});