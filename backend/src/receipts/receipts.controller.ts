...
export class ReceiptsController {
    ...
    @ApiOkResponse({ description: 'Receipt scan review draft returned successfully' })
    @ApiNotFoundResponse({ description: 'Receipt scan does not exist, does not belong to the user, is expired, or has already been consumed' })

    async getScan(@CurrentAuthUser() authUser: AuthUser, @Param('scanId') scanId: string) {
        const user = await this.usersService.findOrCreateUser(authUser);
