...
import {NotificationType,PaymentOccurrenceStatus,PaymentRecordStatus,WagerOutcome,WagerStatus,WagerTaskType} from '@prisma/client';
...
describe('SchedulerService', () => {
  ...
  beforeEach(() => {
    ...
    jest.clearAllMocks();
    remindersService.processDueReminders.mockResolvedValue({processedCount:0});
    paymentOccurrencesService.transitionOverdueOccurrences.mockResolvedValue({transitionedCount:0});
    paymentOccurrencesService.transitionMissedOccurrence.mockResolvedValue({transitionedCount:0});
  ...
  describe('runAll', () => {
    ...
    it('resolves an all payments on time wager with one winner',async()=>{
        ...
        prisma.$transaction.mockImplementationOnce((operation:unknown)=>{
            const callback=operation as (tx:typeof transactionClient)=>Promise<unknown>;
            return callback(transactionClient);
        });
        rewardService.grantCoins.mockResolvedValue({coinBalance:200});
        notificationsService.create.mockResolvedValue(undefined);
        const rslt=await service.runAll();
        expect(rewardService.grantCoins).toHaveBeenCalledWith(
            transactionClient,
            {
                userId:'creator-id',
                amount:100,
    ...
    it('retries settlement after a failed reward transaction',async()=>{
        ...
        notificationsService.create.mockResolvedValue(undefined);
        await expect(service.runAll()).rejects.toThrow('Temporary reward failure');
        const rslt=await service.runAll();
