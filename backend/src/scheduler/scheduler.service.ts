...
import {NotificationType,PaymentOccurrenceStatus,PaymentRecordStatus,Prisma,WagerOutcome,WagerStatus,WagerTaskType} from '@prisma/client';
...
export class SchedulerService {
  ...
  private async expirePendingWagers(now:Date){
    const expiresBefore=new Date(
      now.getTime()-WAGER_INVITE_EXPIRY_MS,
    );
  ...
  private getSnapshotNumber(
    ...
    const value=(taskSnapshot as Prisma.JsonObject)[key];
