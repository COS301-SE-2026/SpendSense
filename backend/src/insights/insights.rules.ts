import { CategoryBreakdownStats, InsightCard, InsightSeverity, ObligationTrendStats, OnTimePaymentStats, PaymentStreakStats, UpcomingPressureStats } from "./insights.types";
...
export function createOnTimePaymentInsight(stats: OnTimePaymentStats,): InsightCard {
    if (!stats.hasEnoughData) {
        return {
            ...
            explanation: "No completed or missed payments were recorded this month yet.",
            severity: "info",
            // link: " ",
        };
    }
    ...
    if (stats.percentagePointChange !== null) {
        ...
        if (change === 0) {
            ...
            trendText = `${Math.abs(change)} percentage points ${change > 0 ? "higher" : "lower"
                } than last month.`;
        }
    }
...
export function createObligationTrendInsight(stats: ObligationTrendStats,): InsightCard {
    if (!stats.hasEnoughData) {
        return {
...
export function createUpcomingPressureInsight(stats: UpcomingPressureStats): InsightCard {
    if (!stats.hasEnoughData) {
        return {
    ...
    let severity: InsightSeverity = "info";
    let comparisonText = "There is not enough recent history for a pressure comparison.";
    ...
    const obligationWord = stats.obligationCount === 1 ? "obligation" : "obligations";

    return {
        key: "upcoming-pressure",
        title: `Next ${stats.windowDays} days`,
        value: formatMoney(stats.upcomingAmount, stats.currency),
        explanation: `${formatNumber(stats.paymentCount)} ${paymentWord} are due across ` + `${formatNumber(stats.obligationCount)} ${obligationWord}. ${comparisonText}`,
        severity,
        // // link: `/payment-occurrences/${id}`, // this should take the user to their payment ocurances - need to fix though
    };
}

export function createCategoryBreakdownInsight(stats: CategoryBreakdownStats): InsightCard {
...
export function createPaymentStreakInsight(stats: PaymentStreakStats,): InsightCard {
    if (!stats.hasEnoughData) {
        return {
            key: "payment-streak",
