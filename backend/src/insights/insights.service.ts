...
import {
  createCategoryBreakdownInsight,
  createObligationTrendInsight,
  createOnTimePaymentInsight,
  createPaymentStreakInsight,
  createUpcomingPressureInsight,
} from './insights.rules';
import {
...
export class InsightsService {
  ...
  private async getOnTimePaymentRate(
    ...
    for (const period of [currentPeriod, previousPeriod]) {
      period.eligiblePaymentCount =
        period.onTimePaymentCount +
        period.latePaymentCount +
        period.missedPaymentCount;
      period.onTimePaymentPercentage =
        period.eligiblePaymentCount === 0
          ? 0
          : roundOne(
              (period.onTimePaymentCount / period.eligiblePaymentCount) * 100,
            );
    }
    const percentagePointChange =
      previousPeriod.eligiblePaymentCount === 0
        ? null
        : roundOne(
            currentPeriod.onTimePaymentPercentage -
              previousPeriod.onTimePaymentPercentage,
          );
  ...
  private async getObligationTrend(
    userId: string,
    asOf: Date,
  ): Promise<ObligationTrendStats> {
    // const userId = await this.resolveUserId(supabaseAuthId);
    ...
    const percentageChange =
      previousMonthAmount === 0
        ? null
        : roundOne((amountChange / previousMonthAmount) * 100);
  ...
  private async getUpcomingPressure(
    userId: string,
    asOf: Date,
  ): Promise<UpcomingPressureStats> {
    // const userId = await this.resolveUserId(supabaseAuthId);
    ...
    const [upcomingOccurrences, recentOccurrences] =
    ...
    const upcomingAmount = roundMoney(
      upcomingOccurrences.reduce(
        (total, occurrence) => total + Number(occurrence.amountDue),
        0,
      ),
    );
    const recentTotal = recentOccurrences.reduce(
      (total, occurrence) => total + Number(occurrence.amountDue),
      0,
    );
    const recentAverageMonthlyAmount = recentTotal / RECENT_MONTH_COUNT;
    const recentAverageWeeklyAmount = roundMoney(
      recentAverageMonthlyAmount / WEEKS_PER_MONTH,
    );
    const pressureRatio =
      recentAverageWeeklyAmount === 0
        ? null
        : roundOne(upcomingAmount / recentAverageWeeklyAmount);
    return {
      currency: INSIGHT_CURRENCY,
      windowDays: UPCOMING_WINDOW_DAYS,
      paymentCount: upcomingOccurrences.length,
      obligationCount: new Set(
        upcomingOccurrences.map((occurrence) => occurrence.obligationId),
      ).size,
      upcomingAmount,
  ...
  private async getCategoryBreakdown(
    userId: string,
    asOf: Date,
  ): Promise<CategoryBreakdownStats> {
    // const userId = await this.resolveUserId(supabaseAuthId);
    const currentStart = startOfUtcMonth(asOf);
    const currentEnd = addUtcMonths(currentStart, 1);
    ...
    const totals = new Map<
      string,
      { amount: number; occurrenceCount: number }
    >();
    ...
    const breakdown = [...totals.entries()]
      .map(([type, item]) => ({
        ...
        percentage:
          currentMonthTotal === 0
            ? 0
            : roundOne((item.amount / currentMonthTotal) * 100),
