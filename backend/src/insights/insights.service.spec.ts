...
describe('InsightsService', () => {
    ...
    describe('resolveUserId', () => {
        ...
        it('Successfulyy throw NotFoundException when the user does not exist', async () => {
            ...
            await expect(service['resolveUserId'](supabaseAuthId)).rejects.toThrow(
                NotFoundException,
            );
            await expect(service['resolveUserId'](supabaseAuthId)).rejects.toThrow(
                'SpendSense user profile could not be found',
            );
        });
    });
        ...
        it('Return all SETTLED payments for the current user', async () => {
            ...
            prismaMock.paymentOccurrence.findMany.mockResolvedValue([
                // FIRST PAYMENT OCCURANCE
                {
                    ...
                    paidAt,
                    obligation: {
                        // OBLIGATION ID FOR THAT PAYMENT OCCURANCE
                        id: 'obligation-1',
                        name: 'Monthly Rent',
    ...
    describe('getSettlementRate', () => {
        it('Calculate percentage of settled payments', async () => {
            ...
            prismaMock.paymentOccurrence.count
                .mockReturnValueOnce(eligibleCountQuery)
                .mockReturnValueOnce(settledCountQuery);
    ...
    describe('getOnTimePaymentRate', () => {
        it('Calculate percentage of on time settled payments', async () => {
            ...
            prismaMock.paymentOccurrence.count
                .mockReturnValueOnce(settledCountQuery)
                .mockReturnValueOnce(onTimeCountQuery);
