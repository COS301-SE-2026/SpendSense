...
describe('Users E2E', () =>{
  it('GET /users/me returns identity, tier, and gamification counters for the scenario user', async () => {
    ...
    try {
      ...
      expect(body.data.gamificationProfile.mascotLevel).toBe(progress.mascotLevel,);
      expect(body.data.gamificationProfile.coinBalance).toBe(progress.coinBalance,);

      expect(body.data.gamificationProfile.currentPaymentStreak).toBe(progress.currentPaymentStreak,);
  ...
  it('GET /users/me isolates users, a token cannot read another user', async ()=> {
    ...
    try {
      const { user: userA } = await createUserWithProfileProgress(e2e.prisma, {email: 'e2e-user-a@example.test',});
      const { user: userB } = await createUserWithProfileProgress(e2e.prisma, {email: 'e2e-user-b@example.test',});
      ...
      expect((response.body as MeResponse).data.user.email).not.toBe(userB.email,);
  ...
  it.skip('PATCH /users/me updates the display name (endpoint pending)', async ()=>{
    ...
    try{
      ...
      const stored = await e2e.prisma.user.findUnique({where: { id: user.id },});
  ...
  it.skip('PATCH /users/me/preferences updates theme and persists (endpoint pending)', async ()=>{
    ...
    try {
      ...
      const stored = await e2e.prisma.userPreference.findUnique({where: {userId: user.id},});
