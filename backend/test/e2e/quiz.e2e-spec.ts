  ...
  it('completes a daily quiz and prevents a second completion', async () => {
    ...
    try {
      ...
      const startResponse = await e2e.request
        ...
        .send({ type: 'DAILY' })
        .expect(201);
      const session = (startResponse.body as ApiEnvelope<QuizSessionResponse>)
        .data;
      ...
      for (let attempt = 0; attempt < 10 && currentQuestion; attempt += 1) {
        ...
        if (!storedQuestion) {
          throw new Error(
            `Seeded quiz question ${currentQuestion.id} was not found.`,
          );
        }
      ...
      if (!finalAnswer?.result) {
        throw new Error(
          'The daily quiz did not complete within five correct answers.',
        );
      }
  ...
  it('lists topics and starts the seeded Credit Score topic quiz', async () => {
    ...
    try {
      ...
      const topicResponse = await e2e.request
        ...
        .set('Authorization', authorization)
        .expect(200);
      const topic = (topicResponse.body as ApiEnvelope<TopicDetailsResponse>)
        .data;
      ...
      expect(topic.teachingContent.title.length).toBeGreaterThan(0);
      expect(typeof topic.teachingContent.body).toBe('string');
      expect(topic.teachingContent.body.length).toBeGreaterThan(0);
      expect(Array.isArray(topic.teachingContent.keyPoints)).toBe(true);
      expect(
        topic.teachingContent.keyPoints.every(
          (keyPoint) => typeof keyPoint === 'string',
        ),
      ).toBe(true);
      const sessionResponse = await e2e.request
        ...
        .expect(201);
      const session = (sessionResponse.body as ApiEnvelope<QuizSessionResponse>)
        .data;
      expect(session).toMatchObject({
        type: 'TOPIC',
        topic: 'CREDIT_SCORE',
