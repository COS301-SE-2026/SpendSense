...
import {
  createFriendshipScenario,
  createThreeUsersForPrivacyFlow,
  createTwoUsersForFriendFlow,
} from '../../../test-support/scenarios/friends-wagers';
...
describe('Friends and Wagers E2E', () => {
  ...
  describe('Friends', () => {
    it('allows one authenticated user to search for another user without exposing private fields', async () => {
      ...
      try {
        const { userA, userB } = await createTwoUsersForFriendFlow(
          e2e.prisma as never,
        );
    ...
    it('completes request, accept, list, detail and removal for two accounts', async () => {
      ...
      try {
        ...
        expect(getResponseData(createResponse.body)).toMatchObject({
          senderId: userA.id,
          receiverId: userB.id,
          status: 'PENDING',
        });

        const requestId = getResponseData<{ id: string }>(
          createResponse.body,
        ).id;
        ...
        expect(getResponseData(outgoingResponse.body)).toEqual(
        ...
        expect(getResponseData(incomingResponse.body)).toEqual(
          expect.arrayContaining([
            expect.objectContaining({
              id: requestId,
              senderId: userA.id,
        ...
        expect(
          getResponseData<{ request: Record<string, unknown> }>(
            acceptResponse.body,
          ).request,
        ).toMatchObject({
          id: requestId,
        ...
        expectPrivacySafeFriend(
          getResponseData<{ friendship: Record<string, unknown> }>(
            acceptResponse.body,
          ).friendship,
        );
        ...
        expect(getResponseData<unknown[]>(listResponse.body)).toHaveLength(1);
        expect(
          getResponseData<Record<string, unknown>[]>(listResponse.body)[0],
        ).toMatchObject({
          friendId: userB.id,
          displayName: 'E2E Friend User B',
        });
        ...
        expect(getResponseData(detailResponse.body)).toMatchObject({
          friendId: userB.id,
          displayName: 'E2E Friend User B',
        });

        expectPrivacySafeFriend(
          getResponseData<Record<string, unknown>>(detailResponse.body),
        );
    ...
    it('prevents an unrelated user from reading another users friend detail', async () => {
      ...
      try {
        ...
        await createMutualFriendship(e2e.prisma as never, userA.id, userB.id);
        ...
        expectErrorEnvelope(response.body as Record<string, unknown>, 404);
      } finally {
        await e2e.close();
      }
    });
    ...
    it('allows an incoming friend request to be declined', async () => {
      ...
      try {
        ...
        expect(getResponseData(declineResponse.body)).toMatchObject({
          id: requestId,
          status: 'DECLINED',
        });
      } finally {
  ...
  describe('Wagers', () => {
    it('creates, lists and retrieves a wager while protecting the detail from unrelated users', async () => {
      ...
      try {
        ...
        expect(getResponseData<unknown[]>(creatorList.body)).toHaveLength(1);
        expect(
          getResponseData<Record<string, unknown>[]>(creatorList.body)[0],
        ).toMatchObject({
          id: wagerId,
          creatorId: userA.id,
          opponentId: userB.id,
        ...
        expect(getResponseData<unknown[]>(opponentList.body)).toHaveLength(1);
        expect(
          getResponseData<Record<string, unknown>[]>(opponentList.body)[0],
        ).toMatchObject({
          id: wagerId,
          creatorId: userA.id,
          opponentId: userB.id,
        ...
        expect(getResponseData(detailResponse.body)).not.toHaveProperty(
          'email',
        );
        expect(getResponseData(detailResponse.body)).not.toHaveProperty(
          'obligations',
    ...
    it('accepts a wager and escrows both stakes without allowing negative balances', async () => {
      ...
      try {
        ...
        expect(
          getResponseData<{ respondedAt: string | null }>(acceptResponse.body)
            .respondedAt,
        ).not.toBeNull();
        expect(
          getResponseData<{ startDate: string | null }>(acceptResponse.body)
            .startDate,
        ).not.toBeNull();
        expect(
          getResponseData<{ endDate: string | null }>(acceptResponse.body)
            .endDate,
        ).not.toBeNull();
        ...
        expect(
          escrowTransactions.map((transaction) => transaction.amount),
        ).toEqual([-50, -50]);
  ...
  describe('Wager settlement', () => {
    it('settles a deterministic winner, conserves coins, creates notifications and cannot settle twice', async () => {
      ...
      try {
        ...
        expect(
          getResponseData<{ resolvedWagerCount: number }>(firstRun.body)
            .resolvedWagerCount,
        ).toBe(1);
        ...
        const transactionsBeforeSecondRun =
          await e2e.prisma.rewardTransaction.count({
            where: {
              userId: {
                in: [userA.id, userB.id],
              },
        ...
        const notificationsBeforeSecondRun =
          await e2e.prisma.notification.count({
            where: {
              type: 'WAGER_RESULT',
              sourceId: wagerId,
            },
        ...
        const transactionsAfterSecondRun =
          await e2e.prisma.rewardTransaction.count({
            where: {
              userId: {
                in: [userA.id, userB.id],
    ...
    it('settles a draw by returning each stake and preserves the total balance', async () => {
      ...
      try {
        ...
        expect(
          profiles.reduce((total, profile) => total + profile.coinBalance, 0),
        ).toBe(400);
        ...
        expect(refunds.map((transaction) => transaction.amount).sort()).toEqual(
          [50, 50],
        );
